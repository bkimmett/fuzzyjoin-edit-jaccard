...
class EditDist { 
	...
	public static int distanceTest(String one, String two, int threshold){
		return distanceTest(one, two, threshold, 2); //default substitution cost is 2
	}

	public static int distanceTest(String one, String two, int threshold, int substitutionCost){
		//using a substitution cost over 2 may yield inaccurate results.
		...
		//System.out.println(progress+":"+Arrays.toString(column)+":"+upper_turn_threshold+":"+(lower_turn_threshold-1));
			...
			for(i=upper_turn_threshold; i<lower_turn_threshold; i++){
				//to the right
				/*if(i >= column.length){
					System.err.println("ERROR: Column is too long. Strings: "+one+" & "+two+"; thresholds = "+upper_turn_threshold+":"+lower_turn_threshold+" at "+i+"; progress = "+progress+"; testing distance "+threshold);
				}*/
				...
				//down and right
				if(i+1 < one.length()+1){
					if(one.regionMatches(true, i /*i-1+1*/, two, progress /*also progress-1+1 */, 1)){
